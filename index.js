...
function MiHumidifier(log, config) {
  ...
  this.log = log;
  this.config = config;
  this.updateTimer = config.updateTimer || 0;
  this.updateInterval = config.updateTimer ? true : false;
  ...
  getServices: function() {
    ...
    humidifierService
      ...
      .on(
        ...
        function(callback) {
          that.device
            .call("get_prop", ["power", "humidity", "child_lock", "dry", "depth", "limit_hum"])
            .then(result => {
      ...
      setInterval(() => {
        that.device
          ...
          .then(result => {
            ...
            if (that.model === "ca1" || "cb1") {
              waterLevel.updateValue(result[4] / 1.2);
            }
            currentHumidityCharacteristic.updateValue(result[1]);
            var relativeHumidityHumidifierThresholdCharacteristic = humidifierService.getCharacteristic(Characteristic.RelativeHumidityHumidifierThreshold);
            relativeHumidityHumidifierThresholdCharacteristic.updateValue(result[5]);
          })
          .catch(function(err) {
            that.log.error("[MiHumidifier][DEBUG] - get Error: " + err);
          });
      }, that.updateTimer * 1000);
    }

    activeCharacteristic.on(
      "set",
      function(value, callback) {
        that.device
    ...
    swingModeControlsCharacteristic.on(
      "set",
      function(value, callback) {
        that.device
      ...
      setInterval(() => {
        that.device
          ...
          .then(result => {
            ...
            if (that.model === "ca1" || "cb1") {
              if (result[0] === "auto") {
                rotationSpeedCharacteristic.updateValue(25);
              } else if (result[0] === "silent") {
                rotationSpeedCharacteristic.updateValue(50);
              } else if (result[0] === "medium") {
                rotationSpeedCharacteristic.updateValue(75);
              } else if (result[0] === "high") {
                rotationSpeedCharacteristic.updateValue(100);
              } else {
                rotationSpeedCharacteristic.updateValue(0);
              }
            } else {
              if (result[0] === "silent") {
                rotationSpeedCharacteristic.updateValue(1);
              } else if (result[0] === "medium") {
                rotationSpeedCharacteristic.updateValue(2);
              } else if (result[0] === "high") {
                rotationSpeedCharacteristic.updateValue(3);
              } else {
                rotationSpeedCharacteristic.updateValue(0);
              }
            }
    ...
    if (!this.config["showTemperatureDisable"]) {
      ...
      if (that.updateInterval) {
        var temperatureSensorCharacteristic = temperatureSensorService.getCharacteristic(Characteristic.CurrentTemperature);
        ...
        setInterval(() => {
          if (that.model !== "cb1") {
            that.device
              ...
              .then(result => {
                that.log.debug("[MiHumidifier][DEBUG]HumidifierDehumidifier - getTemperature: " + result);
                temperatureSensorCharacteristic.updateValue(result[0] / 10);
              })
              .catch(function(err) {
                that.log.debug("[MiHumidifier][DEBUG]HumidifierDehumidifier - getTemperature Error: " + err);
              });
            ...
            that.device
              ...
              .then(result => {
                that.log.debug("[MiHumidifier][DEBUG]HumidifierDehumidifier - getTemperature: " + result);
                temperatureSensorCharacteristic.updateValue(result[0]);
              })
              .catch(function(err) {
                that.log.debug("[MiHumidifier][DEBUG]HumidifierDehumidifier - getTemperature Error: " + err);
              });
